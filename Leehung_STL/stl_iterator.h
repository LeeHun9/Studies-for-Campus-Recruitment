...
迭代器：
1、迭代器本质是指针，该指针比较智能。
2、算法通过迭代器对容器进行操作，算法不必了解容器本身的结构

迭代器的实现位于 stl_iterator.h 和 stl_iterator_base.h

迭代器细分为：
输入迭代器、输出迭代器、正向迭代器、双向迭代器、随机访问迭代器

迭代器中的 iterator_category 和 value_type 只是类型名，SGI_STL使用模板参数推导性质来将信息返回给调用者

Traits技术：
iterator_traits 技术用于萃取出迭代器对应类型
*/
