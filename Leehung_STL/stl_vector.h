...
protected:
    ...
    _Tp* _M_deallocate(_Tp* __p, size_t __n) {
...
public:
    ...
    typedef reverse_iterator<iterator> reverse__iterator;
...
public:
    ...
    reverse__iterator rbegin() {
        return reverse__iterator(end());
    }
    ...
    reverse__iterator rend()
	{
		return reverse__iterator(begin());
	}
...
vector<_Tp, _Alloc>& vector<_Tp, _Alloc>::operator=(const vector<_Tp, _Alloc>& __x) {
    if ( &__x != this ) {
        ...
        if (__xlen > capacity()) {
            ...
            copy(__x.begin(), __x.begin() + __x.size(), _M_start);
